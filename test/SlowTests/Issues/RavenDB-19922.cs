...
namespace SlowTests.Issues
{
    public class RavenDB_19922 : ClusterTestBase
    {
        ...
        public async Task DoNotChangeResponsibleNode(Options options)
        {
            DoNotReuseServer();
            ...
            var (_, leaderServer) = await CreateRaftCluster(clusterSize, customSettings: settings);
        ...
        [RavenTheory(RavenTestCategory.BackupExportImport)]
        [RavenData(SearchEngineMode = RavenSearchEngineMode.All)]
        public async Task ChangeResponsibleNode(Options options)
        {
            DoNotReuseServer();
            ...
            })
            {
                ...
                {
                    if (nodes[i].ServerStore.NodeTag == disposedServer.ServerStore.NodeTag)
                    {
                        index = i;
                        break;
                    }
